...
WITH CTE AS (
	...
	WHERE SUBSTRING(NO_SMOR, 1, 8) BETWEEN '20250226' AND '20250325'),
	 ...
	 -- ✅ NO_SMOR, NO_SEQ, CD_SYSITEM, NO_SMORSUB
		,
	 구매발주정보결합 AS (SELECT O.*,
						 DATEFROMPARTS(2025, MONTH(CONVERT(DATE, DT_DUE, 112)),
									   DAY(CONVERT(DATE, DT_DUE, 112)))   AS 원단납기일자,
						 BC.NM_CUST                                       AS 매입처명,
						 DATEFROMPARTS(2025, MONTH(CONVERT(DATE, DT_ENTER, 112)),
									   DAY(CONVERT(DATE, DT_ENTER, 112))) AS 원단입고일자,
						 CASE
	 ...
	 -- ❤️ 주문원단계수 집계
		,
	 주문원단개수집계 AS (SELECT NO_SMOR,
						 NO_SEQ,
						 SUM(CASE WHEN ITEM_ORDER_ORG = 1 THEN 1 ELSE 0 END)                         AS 주문1급지개수,
						 SUM(CASE WHEN ITEM_ORDER_ORG <> 1 THEN 1 ELSE 0 END)                        AS 주문2급지개수,
						 SUM(CASE WHEN ITEM_ORDER_ORG = 1 AND 원단입고일자 IS NOT NULL THEN 1 ELSE 0 END)  AS 원단1급지개수,
						 SUM(CASE WHEN ITEM_ORDER_ORG <> 1 AND 원단입고일자 IS NOT NULL THEN 1 ELSE 0 END) AS 원단2급지개수,
						 MAX(원단납기일자)                                                                 AS 원단납기일자,
						 MAX(원단입고일자)                                                                 AS 원단입고일자,
						 MAX(원단입고계산)                                                                 AS 원단입고계산
				  FROM 구매발주정보결합
				  GROUP BY NO_SMOR, NO_SEQ)
		,
	 원단정보FINAL AS (SELECT *,
						  DATEDIFF(DAY, 원단입고일자, 원단입고계산) AS 원단지연일수,
						  CASE
...
-- 인쇄, 합지, 분단, 제대 작성
	 ...
	 공정별생산실적일 AS (SELECT A.주문번호,
						 ...
						 A.[04],
						 A.[05],
						 MAX(CASE WHEN TEMP.[02] = A.원단1급지개수 THEN CONVERT(DATE, 생산실적일, 112) ELSE NULL END) AS 인쇄생산실적일,
						 MAX(CASE WHEN TEMP.[03] = A.원단2급지개수 THEN CONVERT(DATE, 생산실적일, 112) ELSE NULL END) AS 합지생산실적일,
						 MAX(CASE
				  ...
				  FROM 주문_원단결합 AS A
						   LEFT JOIN (SELECT *
									  FROM (SELECT NO_SMOR,
												   NO_SEQ,
												   CD_PROCESS,
												   MAX(DT_MMOR)      AS 생산실적일,
												   COUNT(NO_SMORSUB) AS CNT
											FROM MMOR_WORK_M
											WHERE SUBSTRING(NO_SMOR, 1, 8) BETWEEN '20250226' AND '20250325'
				  ...
				  GROUP BY A.주문번호, A.주문세부번호, 발주일자, 제품명, 표준납기일, 주문수량,
						   ...
						   A.[04],
						   A.[05]),
	 CONVERTING AS (SELECT SMOR_ORDER_MATERIAL.NO_SMOR,
						   SMOR_ORDER_MATERIAL.NO_SEQ,
						   SMOR_ORDER_MATERIAL.NO_SMORSUB,
						   SMOR_ORDER_MATERIAL.CD_ITEMGUBUN,
						   BSIT_ITEM.NM_ITEM,
-- 텐덤 등 이름 이상한거 다 바꿈
						   CASE
					...
					WHERE SMOR_ORDER_MATERIAL.CD_ITEMGUBUN = '003'
					  AND SUBSTRING(SMOR_ORDER_MATERIAL.NO_SMOR, 1, 8) BETWEEN '20250226' AND '20250325'),
	 CONVERTING2 AS (SELECT CONVERTING.*,
...
-- 합지 방법별 평균 합지일수 구한 다음에 총 합계 합지생산일수를 주문번호 따라 생성
		 SELECT CONVERTING2.NO_SMOR,
				CONVERTING2.NM_ITEM_MODIFIED,
				AVG(CONVERTING2.합지생산일수) AS 평균_합지생산일수
		 FROM CONVERTING2
		 GROUP BY CONVERTING2.NO_SMOR, CONVERTING2.NM_ITEM_MODIFIED),
	 CONVERTING4 AS (SELECT CONVERTING3.NO_SMOR, SUM(CONVERTING3.평균_합지생산일수) AS 합계_합지생산일수
					 FROM CONVERTING3
					 GROUP BY CONVERTING3.NO_SMOR),
	 공정별계산일수 AS (SELECT 공정별생산실적일.*,
						CONVERTING4.합계_합지생산일수

				 -- 계산된 인쇄, 합지, 분단, 제대 표준납기일 계산
				 -- 이후, 차이를 구해서 지연여부 파악
				 FROM 공정별생산실적일
						  LEFT JOIN CONVERTING4 ON 공정별생산실적일.주문번호 = CONVERTING4.NO_SMOR)
		,
	 공정별입고계산 AS (SELECT 공정별계산일수.*,
-- 인쇄 공정별 계산일수
						CASE
							WHEN 공정별계산일수.[02] IS NOT NULL THEN DATEADD(Day, 3, 원단입고계산)
							WHEN 공정별계산일수.[02] IS NULL THEN 원단입고계산
							ELSE NULL
							END AS 인쇄입고계산,
...
-- 분단 공정별 계산일수
						CASE
							...
							WHEN 공정별계산일수.[02] IS NOT NULL AND 공정별계산일수.[03] IS NOT NULL
								AND 공정별계산일수.[04] IS NULL THEN DATEADD(DAY, 3 + 공정별계산일수.합계_합지생산일수, 원단입고계산)

							WHEN 공정별계산일수.[02] IS NOT NULL AND 공정별계산일수.[03] IS NULL
								AND 공정별계산일수.[04] IS NOT NULL THEN DATEADD(DAY, 4, 원단입고계산)

							WHEN 공정별계산일수.[02] IS NOT NULL AND 공정별계산일수.[03] IS NULL
								AND 공정별계산일수.[04] IS NULL THEN DATEADD(DAY, 3, 원단입고계산)
							--
-- 인쇄 O & 합지 X
...
-- 인쇄 O & 합지 X
							...
							WHEN 공정별계산일수.[02] IS NOT NULL AND 공정별계산일수.[03] IS NULL
								AND 공정별계산일수.[04] IS NOT NULL AND 공정별계산일수.[05] IS NULL THEN DATEADD(DAY, 4, 원단입고계산)

							WHEN 공정별계산일수.[02] IS NOT NULL AND 공정별계산일수.[03] IS NULL
								AND 공정별계산일수.[04] IS NULL AND 공정별계산일수.[05] IS NOT NULL THEN DATEADD(DAY, 6, 원단입고계산)

							WHEN 공정별계산일수.[02] IS NOT NULL AND 공정별계산일수.[03] IS NULL
								AND 공정별계산일수.[04] IS NULL AND 공정별계산일수.[05] IS NULL THEN DATEADD(DAY, 3, 원단입고계산)
...
-- 인쇄 X & 합지 O
							...
							WHEN 공정별계산일수.[02] IS NULL AND 공정별계산일수.[03] IS NOT NULL
								AND 공정별계산일수.[04] IS NOT NULL AND 공정별계산일수.[05] IS NULL
								THEN DATEADD(DAY, 1 + 공정별계산일수.합계_합지생산일수, 원단입고계산)
							...
							WHEN 공정별계산일수.[02] IS NULL AND 공정별계산일수.[03] IS NOT NULL
								AND 공정별계산일수.[04] IS NULL AND 공정별계산일수.[05] IS NULL
								THEN DATEADD(DAY, 공정별계산일수.합계_합지생산일수, 원단입고계산)
...
-- 인쇄 X & 합지 X
							...
							WHEN 공정별계산일수.[02] IS NOT NULL AND 공정별계산일수.[03] IS NULL
								AND 공정별계산일수.[04] IS NOT NULL AND 공정별계산일수.[05] IS NULL THEN DATEADD(DAY, 1, 원단입고계산)

							WHEN 공정별계산일수.[02] IS NOT NULL AND 공정별계산일수.[03] IS NULL
								AND 공정별계산일수.[04] IS NULL AND 공정별계산일수.[05] IS NOT NULL THEN DATEADD(DAY, 3, 원단입고계산)

							WHEN 공정별계산일수.[02] IS NOT NULL AND 공정별계산일수.[03] IS NULL
								AND 공정별계산일수.[04] IS NULL AND 공정별계산일수.[05] IS NULL THEN 원단입고계산
...
SELECT
	temp.*,
	CASE WHEN 인쇄지연일수 <= 0 THEN 0 ELSE 1 END AS 인쇄지연여부,
	CASE WHEN 합지지연일수 <= 0 THEN 0 ELSE 1 END AS 합지지연여부,
	CASE WHEN 분단지연일수 <= 0 THEN 0 ELSE 1 END AS 분단지연여부,
	CASE WHEN 제대지연일수 <= 0 THEN 0 ELSE 1 END AS 제대지연여부

FROM (SELECT *,
			 DATEDIFF(DAY, 인쇄생산실적일, 인쇄입고계산) AS 인쇄지연일수,
			 DATEDIFF(DAY, 합지생산실적일, 합지입고계산) AS 합지지연일수,
			 DATEDIFF(DAY, 분단생산실적일, 분단입고계산) AS 분단지연일수,
			 DATEDIFF(DAY, 제대생산실적일, 제대입고계산) AS 제대지연일수
	  FROM 공정별입고계산) temp
